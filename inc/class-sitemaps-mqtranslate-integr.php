...
if ( ! class_exists( 'WPSEO_Sitemaps_Mqtranslate_Integr' ) ) {
	
	class WPSEO_Sitemaps_Mqtranslate_Integr {
	...
	function __construct() {
		...
		$this->url_mode = $GLOBALS['q_config']['url_mode'];
		$this->default_lang = $GLOBALS['q_config']['default_language'];
		$this->langs = qtrans_getSortedLanguages(); 
		
		remove_action( 'pre_get_posts', array( $GLOBALS['old_wpseo_sitemaps'], 'redirect' ), 1 );
		add_action( 'pre_get_posts', array( $this, 'redirect' ), 1 );
		
		add_filter ( 'wpseo_posts_where', array($this, exclude_unstranslated_posts), 0, 2);
	...
	/**
		...
		function redirect( $query ) {
			...
			// Custom code to add language
			$this->current_lang = $this->get_lang_from_query_var($type);
			if(empty($this->current_lang)){
				$this->current_lang = $this->default_lang;
			}else{
				// Remove the 3 last characters. The lang code is composed by "-" (1 character) + $lang_code (2 characters)
					...
					for ( $i = 0; $i < $n; $i ++ ) {
						...
						if ( empty( $count ) || $count == $n ) {
						 ...
						 foreach ($this->langs as $l){
							...
							$this->sitemap .= '<sitemap>' . "\n";
							$this->sitemap .= '<loc>' . home_url( $base . $post_type . $lang_suffix . '-sitemap' . $count . '.xml' ) . '</loc>' . "\n";
		...
		function sitemap_url( $url ) {
			...
			// Add lang prefix / suffix to the URL
			$url['loc'] = qtrans_convertURL($url['loc'], $this->current_lang);
		...
		/**
		 *  The lang code is attached in the query var called "sitemap".
		 *  This query var attach the following info:
		 *  
		 *  	$post_type  . "-" . $lang_code
		 *  
		 *  In the original function only attach the $post_type.
		 * 
		 */
		public function get_lang_from_query_var($query_var){
			$return = '';
			
			$vars = explode("-", $query_var);
		...
		/**
		 * 	Hook: wpseo_typecount_where
		 * 
		 *  MOdified query to filter posts that don¡t have translations
		 *  
		 *	See qtrans_excludeUntranslatedPosts() in (m)qtranslate_hooks.php
		 * 
		 */
		public function exclude_unstranslated_posts($where_filter, $post_type){
			// $where_filter .= "AND post_content LIKE '%<!--:". $this->current_lang ."-->%'";
			return $where_filter;
		}
